...
In main.m, run `NFS(M,Range,Rank)` where `M` is the adjacency matrix, `Range` is the desired exponent of `q` in the Schur Index, and `Rank` is the rank of the Coulomb Branch.
